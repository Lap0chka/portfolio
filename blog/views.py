    ...
    def post(self, request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponse:
        ...
        if form.is_valid():
            try:
                ...
                messages.error(
                    request, "An error occurred while sending your suggestion email."
                )
...
class DetailPageView(DetailView):
    ...
    def post(self, request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponse:
        ...
        if form.is_valid():
            ...
            last_comment = (
                Comment.objects.filter(user_id=user_id, post=post)
                .order_by("-created_at")
                .first()
            )
            if last_comment and timezone.now() - last_comment.created_at < timedelta(
                minutes=60
            ):
                messages.error(
                    request,
                    "You can only submit a comment once every 60 minutes. "
            ...
            logger.info(
                f"New comment posted by user '{username}' on post '{post.title}'"
            )
        ...
        for error_list in form.errors.values():
            error_message = " ".join(map(str, error_list))
            messages.error(request, error_message)
            logger.error(f"Form submission error: {error_message}")
